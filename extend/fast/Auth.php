 ...
 * 权限认证类
 * 功能特性：
 * 1，是对规则进行认证，不是对节点进行认证。用户可以把节点当作规则名稱实现对节点进行认证。
 *      $auth=new Auth();  $auth->check('规则名稱','用户id')
 * 2，可以同时对多条规则进行认证，并设置多条规则的关系（or或者and）
 *      $auth=new Auth();  $auth->check('规则1,规则2','用户id','and')
 *      第三个参數为and时表示，用户需要同时具有规则1和规则2的权限。 当第三个参數为or时，表示用户值需要具备其中一个条件即可。默认为or
 * 3，一个用户可以屬于多个用户组(think_auth_group_access表 定义了用户所屬用户组)。我们需要设置每个用户组拥有哪些规则(think_auth_group 定义了用户组权限)
 * 4，支持规则表达式。
 *      在think_auth_rule 表中定义一条规则，condition字段就可以定义规则表达式。 如定义{score}>5  and {score}<100
 * 表示用户的分數在5-100之间时这条规则才会通过。
 */
class Auth
{
    ...
    protected $config = [
        'auth_on'           => 1, // 权限開关
        'auth_type'         => 1, // 认证方式，1为实时认证；2为登入认证。
        'auth_group'        => 'auth_group', // 用户组數据表名
    ...
    /**
     * 检查权限
     * @param string|array $name     需要验证的规则列表,支持逗号分隔的权限规则或索引數组
     ...
     * 根据用户id获取用户组,返回值为數组
     * @param int $uid  用户id
     * @return array       用户所屬的用户组 array(
     *                  array('uid'=>'用户id','group_id'=>'用户组id','name'=>'用户组名稱','rules'=>'用户组拥有的规则id,多个,号隔開'),
     *                  ...)
     */
    ...
    public function getRuleList($uid)
    {
        ...
        foreach ($this->rules as $rule) {
            ...
            if (!empty($rule['condition']) && !in_array('*', $ids)) {
                //根据condition进行验证
                $user = $this->getUserInfo($uid); //获取用户信息,一维數组
                $nums = 0;
